...
public class Repository {
     ...
     */
    public Repository() {
        mBoxStore = MyObjectBox.builder().androidContext(App.getApp()).build();

        Box<Zoo> zooBox = mBoxStore.boxFor(Zoo.class);
    ...
    public DataSubscription subscribeToZooList(DataObserver<List<Zoo>> observer) {
        return mBoxStore.boxFor(Zoo.class).query().build().subscribe().on(AndroidScheduler.mainThread()).observer(observer);
    }

    /**
     * Trigger loading of Zoos from the network.
     */
    public void loadZoos() {
        //a network call would go here, which would subsequently save to the database, which automatically triggers observers
        //we could return an observer here to listen for network response if we show a loading spinner, for example and need to dismiss it
    }

    public void addZoo(Zoo newZoo) {
        mBoxStore.boxFor(Zoo.class).put(newZoo);
    }
